...
export default function EventosList() {
  ...
  async function loadEventos() {
    const { data } = await supabase
      .from('eventos')
      .select('*, participantes(id, saldo_pendiente), abonos(monto)')
      .order('fecha', { ascending: false })

    const rows = (data || []).map(e => ({
      ...e,
      participantes_count: e.participantes?.length || 0,
      ingresos_reales: (e.abonos || []).reduce((s, a) => s + Number(a.monto), 0),
      saldo_total: (e.participantes || []).reduce((s, p) => s + Number(p.saldo_pendiente), 0),
    }))
    setEventos(rows)
  }
  ...
  function openEdit(ev) {
    ...
    setEditando(ev.id); setErrors({}); setModalOpen(true)
  }
    ...
    <div style={{ flex: 1 }}>
      ...
      <PageContent>
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
          <SearchBar value={filtro} onChange={setFiltro} placeholder="Buscar por nombre, lugar o ejecutor..." style={{ flex: 1, minWidth: 220 }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
          {filtered.length === 0 ? (
            <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: 48, color: '#6B7B4F', fontFamily: 'DM Sans' }}>Sin eventos encontrados</div>
          ) : filtered.map(ev => {
            const est = getEventoStatus(ev.estado)
            const ocupacion = ev.cupo_maximo > 0 ? Math.round((ev.participantes_count / ev.cupo_maximo) * 100) : null
            const utilidad = Number(ev.ingresos_reales) - Number(ev.costo_total)

            return (
              <div key={ev.id}
                style={{ background: '#fff', borderRadius: 12, border: '1px solid rgba(196,169,125,0.25)', boxShadow: '0 1px 4px rgba(44,58,26,0.06)', overflow: 'hidden', cursor: 'pointer', transition: 'transform 0.15s, box-shadow 0.15s' }}
                ...
                <div style={{ background: ev.tipo === 'caminata' ? '#2C3A1A' : '#4A5E28', padding: '16px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ fontSize: 22, marginBottom: 4 }}>{ev.tipo === 'caminata' ? '🥾' : '✈️'}</div>
                    <div style={{ fontFamily: 'DM Sans', fontSize: 12, color: '#C4A97D', textTransform: 'uppercase', letterSpacing: 0.5 }}>{ev.tipo}</div>
                  </div>
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    <Badge label={est.label} bg={est.bg} color={est.color} />
                    <button onClick={e => { e.stopPropagation(); openEdit(ev) }}
                      style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 6, padding: 6, cursor: 'pointer', color: '#E8C547' }}>
                      <Edit2 size={12} />
                    </button>
                  </div>
                </div>
                ...
                <div style={{ padding: '16px 18px' }}>
                  <h3 style={{ fontFamily: 'Oswald, sans-serif', fontSize: 16, fontWeight: 600, color: '#2C3A1A', margin: '0 0 8px', letterSpacing: 0.3 }}>{ev.nombre}</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#6B7B4F', fontFamily: 'DM Sans' }}><MapPin size={12} color="#4A5E28" />{ev.lugar}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#6B7B4F', fontFamily: 'DM Sans' }}><Users size={12} color="#4A5E28" />{ev.participantes_count} participantes {ev.cupo_maximo > 0 ? `de ${ev.cupo_maximo}` : ''}</div>
                    <div style={{ fontSize: 12, color: '#6B7B4F', fontFamily: 'DM Sans' }}>📅 {formatDate(ev.fecha)} · {ev.ejecutor}</div>
                  </div>
                  ...
                  {ocupacion !== null && (
                    <div style={{ marginBottom: 14 }}>
                      ...
                      <div style={{ height: 6, background: 'rgba(196,169,125,0.2)', borderRadius: 3, overflow: 'hidden' }}>
                        <div style={{ height: '100%', borderRadius: 3, width: `${Math.min(ocupacion, 100)}%`, background: ocupacion >= 90 ? '#E8C547' : '#4A5E28', transition: 'width 0.5s ease' }} />
                      </div>
                    </div>
                  )}

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, borderTop: '1px solid rgba(196,169,125,0.15)', paddingTop: 14 }}>
                    {[
                      ...
                      <div key={label} style={{ textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 700, color, fontFamily: 'Oswald' }}>{formatMXN(val)}</div>
                      </div>
                    ))}
                  </div>

                  {ev.saldo_total > 0 && (
                    <div style={{ marginTop: 10, background: '#FFF3CC', borderRadius: 6, padding: '6px 10px', fontSize: 12, color: '#7A5A00', fontFamily: 'DM Sans', fontWeight: 500 }}>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editando ? 'Editar Evento' : 'Nuevo Evento'} width={580}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <FormField label="Costo total del evento (MXN)" error={errors.costo_total}>
            <Input type="number" min="0" value={form.costo_total} onChange={e => f('costo_total', e.target.value)} placeholder="0.00" />
          </FormField>
          <FormField label="Cupo máximo" error={errors.cupo_maximo}>
            <Input type="number" min="0" value={form.cupo_maximo} onChange={e => f('cupo_maximo', e.target.value)} placeholder="15" />
          </FormField>
          <FormField label="Notas" style={{ gridColumn: '1/-1' }}>
            <Textarea value={form.notas} onChange={e => f('notas', e.target.value)} placeholder="Información adicional del evento..." rows={3} />
          </FormField>
        </div>
